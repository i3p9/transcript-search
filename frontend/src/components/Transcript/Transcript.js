    ...
    const fetchTranscriptData = async () => {
      try {
        ...
        const response = await fetch(
          `${baseUrl}/json/${selectedShow}/${episodeId}.json`
        );
      ...
      {transcriptData?.map((line, index) => {
        if (line.line_number === parseInt(variable)) {
          return (
            //highlighted line
            <div
              className="pt-2 border border-double rounded-md w-70 bg-slate-300"
              key={index}
              id="highlighed-line"
            >
          ...
          return (
            <div
              className="flex flex-col md:flex-row justify-items-start content-evenly border rounded-md"
              key={index}
            >
              <div
